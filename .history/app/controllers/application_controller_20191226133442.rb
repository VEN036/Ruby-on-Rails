class ApplicationController < ActionController::Base
 ...
 # Filter for before configuration reload_rails_admin
 ...
 # Before filter rails admin functionality
	before_filter :configure_permitted_parameters, if: :devise_controller?

	private

	def configure_permitted_parameters
		devise_parameter_sanitizer.for(:sign_up) do |signup_params|
			signup_params.permit :name, :username, :email, :password, :password_confirmation
		end
		devise_parameter_sanitizer.for(:account_update) do |sinup_params|
			signup_params.permit :name, :username, :email, :password, :
		end	
	end
end
